...
class Collect(models.Model):
    ...
    count_people = models.PositiveIntegerField(default=0,
                                               verbose_name="Количество людей "
                                                            "сделавших пожертвований")
    ...
    donates = models.ManyToManyField("payment.Payment", related_name="donates",
                                     verbose_name="Донатеры", blank=True)
    ...
    def save(
        ...
        if self.id:
            ...
            col_list = self.donates.filter(
                status=True
            )
            # получение уникальных id донатеров и их подсчет
            col_list_donat_id = col_list.aggregate(count=Count("sender_id",
                                                               distinct=True))["count"]
            # получение суммы донатов
            col_list_donat_amount = col_list.values_list("amount", flat=True).aggregate(
